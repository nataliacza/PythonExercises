import unittest
from challenges.task_005 import swap_elements, swap_elements_2
...
class TestOfSwapFunctionTwo(unittest.TestCase):
    ...
    def test_function_1(self):
        result = swap_elements_2(list_nr1, 1, 2)
        expected = [4, 6, 5, 7, 1]
        self.assertEqual(result, expected)

    def test_function_2(self):
        result = swap_elements_2(list_nr2, 4, 2)
        expected = ["a", "b", "e", "d", "c"]
        self.assertEqual(result, expected)

    def test_function_3(self):
        result = swap_elements_2(list_nr3, 3, 2)
        expected = [2.1, 5.2, 7.1, 6.2, 2.6, 2.1]
        self.assertEqual(result, expected)
